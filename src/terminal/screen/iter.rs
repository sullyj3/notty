...
/// screen.
pub struct Cells<'a, T: FillPanel + 'a> {
    pub(super) iter: CoordsIter,
    pub(super) section: &'a ScreenSection<T>,
}

impl<'a, T: FillPanel + 'a> Cells<'a, T> {
...
impl<'a, T: FillPanel + 'a> Iterator for Cells<'a, T> {
...
impl<'a, T: FillPanel + 'a> DoubleEndedIterator for Cells<'a, T> {
    fn next_back(&mut self) -> Option<Self::Item> {
        self.iter.next_back().map(|coords| &self.section[coords])
    }
}

impl<'a, T: FillPanel + 'a> ExactSizeIterator for Cells<'a, T> { }

/// An iterator over all of the visible panels in the terminal's screen.
pub struct Panels<'a, T: FillPanel + 'a> {
    pub(super) stack: Vec<&'a ScreenSection<T>>,
}

impl<'a, T: FillPanel + 'a> Iterator for Panels<'a, T> {
    type Item = Cells<'a, T>;
...
impl<'a, T: FillPanel + 'a> ExactSizeIterator for Panels<'a, T> {
    fn len(&self) -> usize {
